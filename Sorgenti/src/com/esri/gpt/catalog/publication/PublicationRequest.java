...
private boolean           _replaceIfExist = true;
...
public void setReplaceIfExist(boolean replaceIfExist) {
  this._replaceIfExist = replaceIfExist;
}
/*ESRI ITALIA 14/03/2017 gestione del flag per la sovrascrittura del metadato*/
...
public void publish() 
  ...
  try {
    ...
    if (getRetryAsDraft()) {
      ...
      Imposto che il documento viene salvato in bozza perchè non validato
      in modo da poter poi visualizzare il messaggio
      */
      /*ESRI 14/03/2017 sostituizione dettata dalle impostazioni utente*/
      getPublicationRecord().setWasDocumentAsDraftWithErrors(true);
    } else {
      throw ex;
    }
    }
}
public void publishSchematronError() 
  throws SchemaException, ImsServiceException, SQLException, CatalogIndexException {
  ...
  try {
    ...
    Schema schema = prepareForPublication();
    publish(schema);
    if (! getPublicationRecord().getWasDocumentNotReplaced()) getPublicationRecord().setWasDocumentAsDraftWithErrors(true);
  } catch (ValidationException ex) {
...
/*ESRI ITALIA 14/03/2017 gestione del flag per la sovrascrittura del metadato*/
  putInfo.setCanReplace(this.getReplaceIfExist());
  /*ESRI ITALIA 14/03/2017 gestione del flag per la sovrascrittura del metadato*/
  ...
  /*ESRI 14/03/2017 sostituizione dettata dalle impostazioni utente*/
  boolean bReplaced2 = imsRequest.getActionStatus().equals(PutMetadataRequest.ACTION_STATUS_NONE_EXIST);
  getPublicationRecord().setWasDocumentNotReplaced(bReplaced2);
  /*ESRI 14/03/2017 sostituizione dettata dalle impostazioni utente*/
  ...
  /*ESRI 14/03/2017 sostituizione dettata dalle impostazioni utente. Se non è stato inserito non deve essere aggiornato*/
  if (!bReplaced2){
