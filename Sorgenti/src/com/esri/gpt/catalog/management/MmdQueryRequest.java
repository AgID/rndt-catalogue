...
public class MmdQueryRequest extends MmdRequest {
  ...
  private boolean                 showListDeleted = false;
  ...
  public String                  csvString = "TITOLO;PROPRIETARIO;STATO;METODO\\n";
...
// methods =====================================================================
  ...
  public void execute() throws SQLException, IdentityException, NamingException,
    ...
    try {
      ...
      s = Val.chkStr(params.getValue("catalog.list.showDeleted"));
      this.showListDeleted = s.equalsIgnoreCase("true");
      ...
      String sUserTable = colDao.getUserTableName();
      String sPATable = colDao.getPATableName();
      ...
      //Show the elemente deleted
      //Esri 13/03/2017
      if (! this.showListDeleted){
        if (sbWhere.length() > 0) sbWhere.append(" AND ");
        sbWhere.append("(NOT (A.APPROVALSTATUS IN ('"+  MmdEnums.ApprovalStatus.disapproved  +"')))");
      }
      // collection filter
      ...
      // collection filter
      String sColEnte = Val.chkStr(criteria.getEnte());
      if (sColEnte.length() > 0) {

        //String sUserTable = colDao.getUserTableName();
        //String sPATable = colDao.getPATableName();
        /*
        SELECT * FROM geoportal.gpt_resource WHERE OWNER in (SELECT USERID FROM geoportal.gpt_user WHERE
    FK_IDPA in (SELECT ID FROM geoportal.gpt_pa WHERE nome = "Regione Autonoma Valle d'Aosta"));
        */

        String subPA = "SELECT ID FROM "+sPATable+" WHERE nome = \""+sColEnte+"\"";
        String subUser = "SELECT USERID FROM "+sUserTable+" WHERE FK_IDPA in ("+subPA+")";
        if (sbWhere.length() > 0) sbWhere.append(" AND ");
        sbWhere.append("(A.OWNER IN ("+subUser+"))");
      }
  ...
  /*private void csvBuilder(PreparedStatement st){ 
    try{
      ResultSet rs = st.executeQuery();
      ManageMetadataController.csvQuery = "TITOLO;PROPRIETARIO;STATO;METODO;DATA\\n";
      while(rs.next()){
        ManageMetadataController.csvQuery += rs.getString("TITLE") + ";";
        ManageMetadataController.csvQuery += rs.getString("OWNER") + ";";
        ManageMetadataController.csvQuery += rs.getString("APPROVALSTATUS") + ";";
        ManageMetadataController.csvQuery += rs.getString("PUBMETHOD") + ";";
        ManageMetadataController.csvQuery += rs.getString("UPDATEDATA") + "\\n";
      }
