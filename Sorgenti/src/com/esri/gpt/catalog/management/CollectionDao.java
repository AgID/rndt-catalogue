...
public class CollectionDao extends BaseDao {
  ...
  public int removeMembers(Publisher publisher, StringSet docUuids, String colUuid) 
    ...
    public String insertCollectionResource(String resouceID, String CollectionID,Publisher pp)throws SQLException, IOException, CatalogIndexException {
       //public int addMembers(Publisher publisher, StringSet docUuids, String colUuid) 
        String ret="0";
        //Check if id ai already inserted
        PreparedStatement st = null;
        boolean continua=true;
        StringSet elencoR =  new StringSet();
        String sql = "SELECT count(*) FROM gpt_collection_member c  WHERE c.COLUUID = '" + CollectionID + "' AND DOCUUID='" + resouceID + "'";
        this.logExpression(sql);
        Connection con = this.returnConnection().getJdbcConnection();
        st = con.prepareStatement(sql);
        ResultSet rs = st.executeQuery();
        ...
        if (continua){
          //Can Insert
            elencoR.clear();
            elencoR.add(resouceID);

            addMembers(pp, elencoR, CollectionID);
            /*PreparedStatement st2 = null;
            try {
                String sql2 = "INSERT INTO gpt_collection_member (COLUUID,DOCUUID) VALUES (?,?)";

                st2 = con.prepareStatement(sql2);
                st2.setString(1,CollectionID);
                st2.setString(2,resouceID);
                st2.addBatch();
                st2.executeBatch();
            } finally {
                CollectionDao.closeStatement(st);
