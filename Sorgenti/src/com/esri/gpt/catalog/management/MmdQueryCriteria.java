...
// properties ==================================================================
  ...
  public Map<String,Object> appendWherePhrase(RequestContext context, String tableAlias, StringBuilder wherePhrase, Publisher publisher) {
    ...
    if (sTitle.length() > 0) {
        //Modifica ESRI Italia per ricerca in Like sul titolo
        // 03/03/2017
      boolean bForceLike = true;
    ...
    if (sDocUuid.length() > 0) {
      ...
      //Evoluzione futura
      //sDocUuid = appendValueFilter(wherePhrase, tableAlias+ "FILEIDENTIFIER", sDocUuid, false);
      //Modifica ESRI ITALIA
      //ID METEDATO corrisponde al file identifier
    ...
    if (owner!=null) {
     ...
     // Esri Italy appendExpression(wherePhrase, "" +tableAlias+ "OWNER = ?");
        if (!publisher.getIsAdministrator()){
            appendExpression(wherePhrase, "" +tableAlias+ "OWNER in (select u2.userid from gpt_user u inner join gpt_user u2 on u2.FK_IDPA=u.FK_IDPA where u.USERID=?)");
            if (publisher!=null) {
                args.put("nUserId", new Integer(publisher.getLocalID()));
            } else {
                args.put("nUserId", 0);
            }
        }
