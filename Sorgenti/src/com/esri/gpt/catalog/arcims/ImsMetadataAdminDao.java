...
public int deleteRecord(String uuid) 
  ...
  try {
    ...
    /* Modified by Esri Italy: Cancellazione del record dalle statistiche */
    // gpt_resource_stat 
    sSql = "DELETE FROM gpt_resource_stat WHERE DOCUUID=?";
    logExpression(sSql);
    st = con.prepareStatement(sSql);
    st.setString(1,uuid);
    st.executeUpdate();
    ...
    sSql = "DELETE FROM gpt_resource_stat_topic WHERE DOCUUID=?";
    logExpression(sSql);
    st = con.prepareStatement(sSql);
    st.setString(1,uuid);
    st.executeUpdate();
    ...
    sSql = "DELETE FROM gpt_resource_stat_service_type WHERE DOCUUID=?";
    logExpression(sSql);
    st = con.prepareStatement(sSql);
    st.setString(1,uuid);
    st.executeUpdate();
    ...
    sSql = "DELETE FROM gpt_resource_stat_inspire_theme WHERE DOCUUID=?";
    logExpression(sSql);
    st = con.prepareStatement(sSql);
    st.setString(1,uuid);
    st.executeUpdate();
    ...
    sSql = "DELETE FROM gpt_resource_stat_inspire_service WHERE DOCUUID=?";
    logExpression(sSql);
    st = con.prepareStatement(sSql);
    st.setString(1,uuid);
    st.executeUpdate();
...
public int deleteRecord(Publisher publisher, MmdQueryCriteria criteria) throws Exception {
  ...
  try {
    ...
    /*TABELLE STATISTICHE ESRI*/
    sbData = new StringBuilder();
    sbData.append(" DELETE  FROM gpt_resource_stat_topic  WHERE id in (SELECT t.ID  FROM gpt_resource_stat_topic t left join gpt_resource r on r.DOCUUID = t.DOCUUID where r.ID is null)");
    st = con.prepareStatement(sbData.toString());
    criteria.applyArgs(st, 1, args);
    logExpression(sbData.toString());
    st.executeUpdate();
    ...
    sbData = new StringBuilder();
    sbData.append(" DELETE  FROM gpt_resource_stat_inspire_theme  WHERE id in (SELECT t.ID  FROM gpt_resource_stat_inspire_theme t left join gpt_resource r on r.DOCUUID = t.DOCUUID where r.ID is null)");
    st = con.prepareStatement(sbData.toString());
    criteria.applyArgs(st, 1, args);
    logExpression(sbData.toString());
    st.executeUpdate();
    ...
    sbData = new StringBuilder();
    sbData.append(" DELETE  FROM gpt_resource_stat_service_type  WHERE id in (SELECT t.ID  FROM gpt_resource_stat_service_type t left join gpt_resource r on r.DOCUUID = t.DOCUUID where r.ID is null)");
    st = con.prepareStatement(sbData.toString());
    criteria.applyArgs(st, 1, args);
    logExpression(sbData.toString());
    st.executeUpdate();
    ...
    sbData = new StringBuilder();
    sbData.append(" DELETE  FROM gpt_resource_stat  WHERE id in (SELECT t.ID  FROM gpt_resource_stat t left join gpt_resource r on r.DOCUUID = t.DOCUUID where r.ID is null)");
    st = con.prepareStatement(sbData.toString());
    criteria.applyArgs(st, 1, args);
    logExpression(sbData.toString());
    st.executeUpdate();
...
public int updateApprovalStatus(Publisher publisher, 
  ...
  try {
    if (!uuids.isEmpty()) {   
      ...
      sbSql.append(" WHERE DOCUUID IN (").append(generateQMarks(uuids.size())).append(")");
      if (! MmdEnums.ApprovalStatus.incomplete.equals(approvalStatus))  sbSql.append(" AND (APPROVALSTATUS IS NULL OR APPROVALSTATUS <> ?)");
      logExpression(sbSql.toString());
      st = con.prepareStatement(sbSql.toString());
      argIndex = 0;
      ...
      if (! MmdEnums.ApprovalStatus.incomplete.equals(approvalStatus))  st.setString(++argIndex,MmdEnums.ApprovalStatus.draft.toString());
      nRows = st.executeUpdate();
      
      // re-build the index uuid set if 'draft' documents were not updated
      if (this.hadUnalteredDraftDocuments || (nRows != uuids.size())) {
        closeStatement(st);
        /*ESRI modifiche per richiesta cancellazione 24/03/2017*/
        if (MmdEnums.ApprovalStatus.incomplete.equals(approvalStatus)) {
            if (nRows == uuids.size()) this.hadUnalteredDraftDocuments=false;
        }
        /*ESRI modifiche per richiesta cancellazione 24/03/2017*/
        
        //Perchè il clear se dopo viene utilizzato???
        //uuids.clear();
        sbSql = new StringBuffer();
        sbSql.append("SELECT DOCUUID FROM ").append(getResourceTableName());
        sbSql.append(" WHERE DOCUUID IN (").append(generateQMarks(uuids.size())).append(")");
