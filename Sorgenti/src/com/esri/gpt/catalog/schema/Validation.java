...
/*	Added by ESRI Italy: espressione regolare universale*/
...
public Validation(Validation objectToDuplicate) {
  ...
  if (objectToDuplicate != null) {
    ...
    /*	Added by ESRI Italy: settaggio espressione regolare*/
    this.setRegExp(objectToDuplicate.getRegExp());
  }
}
...
public void setRegExp(String regExp) {
	if(regExp.equals(""))	//assume questo valore sia se regexp non e' presente, sia se il suo valore e' la stringa vuota	
		_regexp = "";
	else
		_regexp=regExp;
}
...
public void configure(CfgContext context, Node node, NamedNodeMap attributes) {
  ...
  /*Added by ESRI Italy: legge espressione regolare da validare*/
  setRegExp(DomUtil.getAttributeValue(attributes,"regexp"));
}
...
public void validate(Schema schema, Section section, Parameter parameter) {
  ...
  if (getRequired() && bIsEmpty) {
    ...
    // check an Integer type
	  ...
	  *	The type is right. Now control if the value match with the *
	  * regexp associated in xml schema definition, if exists	   *
	  */
    if((this.getRegExp()!="") && (!sValue.matches(this.getRegExp()))){
        errors.addInvalidParameter(section,parameter);
	 }
  } 
