...
public boolean canHandleResource() {
  ...
  */
  if (hintServiceType.equalsIgnoreCase(ResourceLinkBuilder.ServiceType.WMS
      .name())
      || hintServiceType.equalsIgnoreCase(ResourceLinkBuilder.ServiceType.WFS
    	      .name())
      || hintServiceType.equalsIgnoreCase(ResourceLinkBuilder.ServiceType.WCS
    	      .name())
  ...
  /****************Modified By Esri Italy******
  /**
  * This ois the AGS case. It will be distinguished in standard MapServer, ImageServer, 
  * InspireView and WMTS
  */ 
  if (hintServiceType.equalsIgnoreCase(ResourceLinkBuilder.ServiceType.AGS
      .name())) {
    if (resourceUri.toLowerCase().contains("/rest")){
    
    	// The standard MapServer case is ONLY when MapSrver is at the end of the resource url
       
      if (resourceUri.toLowerCase().endsWith("/inspireview")) {
          this._hintServiceType = "inspireview";
          LOG.finer("Could handle resrouceuri " + _resourceUri +" as Inspire View service");
          return true;
        }
      if (resourceUri.toLowerCase().endsWith("/wmts")) {
    	  if(resourceUri.toLowerCase().contains("/mapserver")){	
    		this._hintServiceType = "wmtsmap";
          	LOG.finer("Could handle resrouceuri " + _resourceUri +" as WMTS map service");
          	return true;
          }
          if(resourceUri.toLowerCase().contains("/imageserver")) {
        	  	this._hintServiceType = "wmtsimage";
            	LOG.finer("Could handle resrouceuri " + _resourceUri +" as WMTS image service");
            	return true;
          	}
          }
	      if (resourceUri.toLowerCase().endsWith("/imageserver")) {
	          this._hintServiceType = "imageserver";
	          LOG.finer("Could handle resrouceuri " + _resourceUri +" as image service");
	          return true;
	        }
	        if (resourceUri.toLowerCase().endsWith("/mapserver")) {
      this._hintServiceType = "agsrest";
	      	  LOG.finer("Could handle resrouceuri " + _resourceUri+ " as mapserver");
      return true;
    }
          return true;
       }
