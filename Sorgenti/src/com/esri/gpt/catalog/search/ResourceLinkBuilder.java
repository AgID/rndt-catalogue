...
/** instance variables ====================================================== */
/** Modified by Esri Italy: changed variables from finel to normal **/
/**
...
protected String              externalMetadataPath = "/catalog/search/dsFullMetadata.page"; //scheda metadati esterni
private final String              imagePath            = "/catalog/images";
private String                    mapViewerUrl         = "";
private final String              metadataPath         = "/rest/document";
private String              previewPath          = "/catalog/livedata/preview.page";  // preview interni
private String              resourceDetailsPath  = "/catalog/search/resource/details.page";  //scheda metadati interni
private String              resourcePreviewPath          = "/catalog/search/resource/livedata-preview.page";  //preview esterni
...
public void build(SearchXslRecord xRecord, SearchResultRecord record) {
  ...
  /** 
   *Riordino dei link associati ai record
   *di metadati 						  
...
protected void buildDetailsLink(SearchXslRecord xRecord, SearchResultRecord record) {
  ...
  String resourceUrl = "";
    ...
    /** Addded by Esri Italy: if resourceDetailPath is absolute use it. **/
    if (this.resourceDetailsPath.startsWith("http")){
        resourceUrl = this.resourceDetailsPath;
    } else {
        resourceUrl = this.getBaseContextPath() + this.resourceDetailsPath;
    }  
    /** End **/
 ...
 */
    ...
    resourceUrl = resourceUrl + "?uuid="
    + encodeUrlParam(uuid);     
  }
  /*ADDED By Esri Italy*/
  /**
   * Se il record e' esterno, l'url relativo viene recuperato tramite l'uuid e l'id del catalogo stesso. 
   * L'url recupevato viene poi passato nel parametro catalog, mentre l'uuid e l'id del catalogo vengono passati tramite i parametri
   * extid e rid. Viene inoltre passato ache il titolo del metadato, per facilitare la visualizzazione allo step successivo.
   */
   else{
	  try{
	  ...
	  engine = SearchEngineFactory.createSearchEngine(
	          ...
	          this.getRequestContext(), record.getExternalId(), 
	          this.getMessageBroker());
	  resourceUrl=resourceUrl + "?catalog="
	    +engine.getMetadataUrl(uuid).replace("&", "@amp;")+"&rid="+record.getExternalId()+"&extid="+record.getUuid()+"&title="+record.getTitle();
	  }catch(Exception ex){
		  LOG.fine("Problema");
	  }
  ...
  if (uuid.length() > 0) {
    if (record.isExternal()) {
      ...
      // we'd have to broker the authentication
        ...
        // Added by Esri Italy: if externalMetadataPath is absolute then do not add baseContextPath
        // This check is very basic though
      if (this.externalMetadataPath.startsWith("http")){
          url = this.externalMetadataPath;
      } else {
          url = this.getBaseContextPath() + this.externalMetadataPath;
      }
      url = url + "?uuid="
          + encodeUrlParam(uuid) + "&rid="
          + encodeUrlParam(record.getExternalId());
    } else {
        if (this.metadataPath.startsWith("http"))
           url = this.metadataPath;
        else
           url = this.getBaseContextPath() + this.metadataPath;
      url = url + "?id=" + encodeUrlParam(uuid);
    }
/******      
...
protected void buildPreviewLink(SearchXslRecord xRecord, SearchResultRecord record) {
  ...
  String url ="";
  /** Added by Esri Italy if url is absolute use is as it is **/
  if(record.isExternal()){
	  ...
	  url = this.getBaseContextPath() + previewPath;
          ...
          if (resourcePreviewPath.startsWith("http")){
            url = resourcePreviewPath;
  }else{
...
//esri italy
  url=url+"&extent="+xRecord.getEnvelope();
  if (serviceType.length() > 0) {
...
//modified by esri italy
  List<String> schemeValsOtherLinks = references.get(Scheme.SERVER.getUrn());
  //pulisco la lista da eventuali doppioni
  for(int i=0;i<schemeValsOtherLinks.size();i++){
	  String app=schemeValsOtherLinks.get(i);
	  for(int j=0;j<schemeValsOtherLinks.size();j++){
		  if(app.equals((String)schemeValsOtherLinks.get(j)) && (i!=j)){
			  schemeValsOtherLinks.remove(j);
		  }
	  }
  }
  ...
  if (schemeVals.size() > 0) {
    ...
    schemeValsOtherLinks.remove(0);//modified by esri Italy: elimino il primo servizio della lista, visto che e' gia' stato preso in considerazoine
  }
...
//modified by Esri Italy: aggiunta degli altri links per visualizzazione in json
  ResourceLinks rls=new ResourceLinks();
  Iterator<String> i=schemeValsOtherLinks.iterator();
  while (i.hasNext())
  {	  
	  ResourceLink rl=new ResourceLink();
	  rl.setUrl(i.next().toString());
	  rl.setTag("other");
	  rls.add(rl);
  }
  record.setResourceLinks(rls);
...
public void initialize(HttpServletRequest request, RequestContext context,
  ...
  this.mapViewerUrl = context.getCatalogConfiguration().getSearchConfig()
      .getMapViewerUrl();
  /** Added by Esri Italy: additional customizable pages **/
  /* For each string if epty assign a default value (set a the beginning of ths file) **/
  String appo;
  appo = context.getCatalogConfiguration().getSearchConfig().getExternalMetadataPath();
  this.externalMetadataPath = (appo.isEmpty()) ? this.externalMetadataPath : appo;
  appo = context.getCatalogConfiguration().getSearchConfig().getResourceDetailsPath();
  this.resourceDetailsPath = (appo.isEmpty()) ? this.resourceDetailsPath : appo;
  appo = context.getCatalogConfiguration().getSearchConfig().getPreviewPath();
  this.previewPath =  (appo.isEmpty()) ? this.previewPath : appo;
  appo = context.getCatalogConfiguration().getSearchConfig().getResourcePreviewPath();
  this.resourcePreviewPath =  (appo.isEmpty()) ? this.resourcePreviewPath : appo;
  /** End Added by Esri Italy **/
