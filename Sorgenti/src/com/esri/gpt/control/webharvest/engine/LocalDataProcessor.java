    ...
    try {
      ...
      if (proceed) {
        ...
        if (!fileSch.isEmpty()){ 
            ...
            LOGGER.finer("[SYNCHRONIZER] file to be validated against schematron:" + publicationRequest.getPublicationRecord().getSourceXml());

            ResourcePath rp2 = new ResourcePath();
            URL url = rp2.makeUrl(fileSch);
            InputStream is  = url.openStream();
            fromMaven OUT = new fromMaven();
            InputStream fileVal = new ByteArrayInputStream(publicationRequest.getPublicationRecord().getSourceXml().getBytes("UTF-8"));
            ...
            if (OUT.failedList.isEmpty()) {
                ...
                for (SVRLFailedAssert failedAssert : OUT.failedList) {
                    //lstErr.add("Test: " + failedAssert.getTest() + " - FALLITO (Valore=" + failedAssert.getText() + ")");
                    lstErr.add("Errore: " + failedAssert.getText());
                }
                rp.createInvalidEntry(sourceUri.asString(), lstErr);
                listener.onPublishException(unit.getRepository(), sourceUri, metadata, null);
            }
        }

         if (continua){
          //FINE Modifiche EsriItalia per validazione Schematron
            publicationRequest.publish();
            boolean bReplaced = publicationRequest.getPublicationRecord().getWasDocumentReplaced();
