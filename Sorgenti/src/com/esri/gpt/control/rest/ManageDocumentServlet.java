  ...
  private void executePut(HttpServletRequest request, HttpServletResponse response,
    ...
    if (record==null) {
      ...
      if (xml.length() > 0) {
        ...
        try {
        // Esri Italy: also editing passes schematron validation (if parameter "publicationRequest.schematron" is "true") default is "false"
        /// Get the schematron parameter
            StringAttributeMap params = context.getCatalogConfiguration().getParameters();
            String schematronYorN = Val.chkStr(params.getValue("publicationrequest.schematron"));
            if (schematronYorN.equalsIgnoreCase("true")) {
                Schema schema= pubRequest.prepareForPublication();
                String fileSch = schema.getSchematronSch();
                if (!asDraft.equals("true")) {
                    if (!fileSch.isEmpty()){
                        InputStream fileVal  = new ByteArrayInputStream(xml.getBytes("UTF-8"));
