...
private void executeHistory(ActionEvent event, RequestContext context)
  ...
  if (params.containsKey("catalog.urlhistory")) aout= params.getValue("catalog.urlhistory").toString();
  if (aout.equals("")) {
      setNavigationOutcome("catalog.harvest.manage.history");
  } else {
      aout +=getOpenDocumentUuid();
      aout ="window.parent.location='" + aout + "';";
      setNavigationOutcome("catalog.harvest.manage.history.redirect");
      context.getServletRequest().setAttribute("location",aout);
  }
