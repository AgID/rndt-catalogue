...
// properties ==================================================================
public void setdatiFromURL(String valore){
...
public String getListType() {
  RequestContext context = this.getContextBroker().extractRequestContext();
  StringAttributeMap params = context.getCatalogConfiguration().getParameters();
  String listType = Val.chkStr(params.getValue("catalog.list.version"));
  if (!((listType.contains("standard")) || (listType.contains("agid")) ||(listType.contains("pa")))) listType="standard";
  return listType;
}
...
private void executeSearch(ActionEvent event, RequestContext context,
  ...
  // check the publisher
  if (!publisher.getIsAdministrator()) {
    //Modifica ESRI per permettere all'utente publisher di vedere anche i metatdati degli altri della sua PA
...
private void executeLink(ActionEvent event, RequestContext context, MmdActionCriteria actionCriteria, Publisher pp) throws Exception {
            CollectionDao colDao = new CollectionDao(RequestContext.extract(null));
            try {
                String[] valori=this._datiFromURL.split("@@@");
                String[] resource=valori[1].split(",");
                String[] collection = valori[0].split(",");
                int conta=0;
                for (int i=0;i<resource.length;i++){
                    for (int y=0;y<collection.length;y++){
                        
                        if (colDao.insertCollectionResource(resource[i], collection[y],pp).equals("0")) conta++;
                    }
                }
                if (conta>0){
                      extractMessageBroker().addSuccessMessage(
                    "catalog.publication.manageMetadata.action.success",
                     new Object[]{Integer.toString(conta)});
                }
...
} else if (sCommand.equals("linkResource")) {
    executeLink(event, context, actionCriteria,publisher);
  }
