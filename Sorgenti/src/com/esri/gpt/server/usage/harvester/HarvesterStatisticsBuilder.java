	...
	public void buildStatistics() throws Exception {
		try {
			...
			String uuids = Val.chkStr(params.getValue("uuids"));
                        String harvest_id = Val.chkStr(params.getValue("harvestids"));
			String startDate = Val.chkStr(params.getValue("start-date"));
			String endDate = Val.chkStr(params.getValue("end-date"));
                        String owner = Val.chkStr(params.getValue("owner"));
                        String lstStat=Val.chkStr(params.getValue("tipo_stat"));
                        String datiPA=Val.chkStr(params.getValue("dati_pa"));
                        String datiTipoPA=Val.chkStr(params.getValue("dati_type"));
			...
			if (restUriParts.length >= 6 && restUriParts[5].equalsIgnoreCase(StatisticsDimensions.SUMMARY.toString())) {
				sb.append("\"summary\": {");
				addSpaces(3);
				buildSummary();
				addSpaces(2);
				...
				sb.append("}");
				addSpaces(1);
			} else if (restUriParts.length >= 6 && restUriParts[5].equalsIgnoreCase(StatisticsDimensions.PENDING.toString())) {
				if(hasConfigInfo){
					sb.append(",");
					addSpaces(2);
				}
				...
				sb.append("}");
				addSpaces(1);
			} else if (restUriParts.length >= 6 && restUriParts[5].equalsIgnoreCase(StatisticsDimensions.COMPLETED.toString())) {
				if(hasConfigInfo){
					sb.append(",");
					addSpaces(2);
				}
				...
				sb.append("}");
				addSpaces(1);
			} else if (restUriParts.length >= 6 && restUriParts[5].equalsIgnoreCase(StatisticsDimensions.HISTORY.toString())) {
				if(hasConfigInfo){
					sb.append(",");
					addSpaces(2);
				}
				...
				String sql = harvesterStatDao.createSelectHistorySQL(uuids,
						startDate, endDate,harvest_id);
				harvesterStatDao.fetchHistory(sql, uuids, startDate, endDate);
				addSpaces(2);
                                sb.append(",");
			        sql = harvesterStatDao.createSelectReportSQL("HARVEST_ID");
				sb.append(harvesterStatDao.fetchtReportSQL(sql, uuids,false));
				addSpaces(1);
				sb.append("}");
				addSpaces(1);
                        } else if (restUriParts.length >= 6 && restUriParts[5].equalsIgnoreCase(StatisticsDimensions.REPORT.toString())){
				String sql = harvesterStatDao.createSelectReportSQL("UUID");
				sb.append(harvesterStatDao.fetchtReportSQL(sql, uuids,true));
				addSpaces(1);
			} else if (restUriParts.length >= 6 && restUriParts[5].equalsIgnoreCase(StatisticsDimensions.LIST.toString())) {                                
                                sb.append(harvesterStatDao.createAndFetchSelectGroupByMetadataSQL(owner,lstStat, datiPA ,datiTipoPA, startDate, endDate));
				...
				if (metrics.contains(StatisticsMetrics.DATABASE.toString()
					...
					if (dimensions.contains(StatisticsDimensions.HISTORY
						...
						String sql = harvesterStatDao.createSelectHistorySQL(
								uuids, startDate, endDate,harvest_id);
