  ...
  private void authorizeResourceAccess(AsnContext context) 
    ...
    if (!context.getOperation().getAuthPolicy().getAllowNonLocalResourceIds()) {
      ...
      if (ts == null) {
    	  ...
    	  /**
    	   * di base, nel resourceId, lui si aspetta di trovare l'uuid.
    	   * Spesso, pero', per le modifiche fatte ai profili, si trova il
    	   * file identifier. Cio' compromette la corretta visualizzazione della pagina dei commenti.
    	   * In questi casi, quindi, va recuperato l'uuid del documento.
    	   */
    	ImsMetadataAdminDao adminDao= new ImsMetadataAdminDao(rContext);
    	String Uuid=null;
    	try{
    		Uuid = adminDao.findExistingUuid(resourceId, null);
    	}
    	catch(Exception ex)
    	{	throw new CatalogIndexException("Invalid resource id: "+resourceId);
    	}
    	Timestamp ts1 = adapter.queryModifiedDate(Uuid);
    	if(ts1==null)
        throw new CatalogIndexException("Invalid resource id: "+resourceId);
      }
    }
