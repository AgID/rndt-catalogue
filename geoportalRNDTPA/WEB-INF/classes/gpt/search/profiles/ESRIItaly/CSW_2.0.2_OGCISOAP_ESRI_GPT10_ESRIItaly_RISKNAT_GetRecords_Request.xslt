...
<!-- La ricerca viene  filtrata sulle parole chiave definite per Risknat (che sono due temi INSPIRE).
Se  viene inserito un termine di ricerca allora si aggiunge al filtro -->
		<xsl:if test="normalize-space(.)!=''">
            <ogc:PropertyIsLike wildCard="" escape="" singleChar="">
                <ogc:PropertyName>AnyText</ogc:PropertyName>
                <ogc:Literal>
                    <xsl:value-of select="."/>
					AND ("geoscientificInformation" OR "Area management/restriction/regulation zones and reporting units")
				</ogc:Literal>
            </ogc:PropertyIsLike>
        </xsl:if>
		<xsl:if test="normalize-space(.)=''">
            <ogc:PropertyIsLike wildCard="" escape="" singleChar="">
                <ogc:PropertyName>AnyText</ogc:PropertyName>
                <ogc:Literal>
                    "geoscientificInformation" OR "Area management/restriction/regulation zones and reporting units"
				</ogc:Literal>
            </ogc:PropertyIsLike>
        </xsl:if>
	</xsl:template>
