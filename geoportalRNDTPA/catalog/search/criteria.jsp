  ...
  Initialization of page components
    ...
    **/
    function scReconfigureCriteria() {
	  ...
	  //added by Esri Italy
	  el = document.getElementById("frmSearchCriteria:_pngProtocol");
      scShow(!blnRemoteCatalog, el);
    }
      ...
      for(var i = 0; i < _scSearchSites.rows.length; i++) {
        ...
        try {
          ...
          if(bCheck == true) {
            ...
            "<input id=\""+ siteId+"\" styleClass=\"locatorInput form-control\" "+ disabled + " type=\"checkbox\" name=\"remoteCatalog\" onclick=\"return scWriteToDistrEndPointComponents(this);\" checked=\"true\" >");
          } else {
            el = document.createElement(
            "<input id=\""+ siteId+"\" styleClass=\"locatorInput form-control\" " + disabled + " type=\"checkbox\" name=\"remoteCatalog\" onclick=\"return scWriteToDistrEndPointComponents(this);\" >");
          }
        } catch(err)  {
...
/** This part allows for multilingual search. The input text is prefixed with the
multi adaptor. 
WARNING: The name of the adaptor must mutch the name written in config file (gpt.xml) **/
/* Actual name of the adaptor: babel **/
	var multiAdaptor='babel|';
    var valore=dojo.byId('frmSearchCriteria:scText').value;
	// If multilingual search is checked, and search text is not null and the adaptor is not there then add the adaptor
    if ((dojo.byId('frmSearchCriteria:scMulti').checked==true) && (valore != '') && (!(valore.startsWith(multiAdaptor))))
        valore= multiAdaptor + valore;
 
    var scText = GptUtils.valChkStr(valore);
//    var scText = GptUtils.valChkStr(dojo.byId('frmSearchCriteria:scText').value);
/* End of multilingual part *********************/

	var scProtocolHidden=GptUtils.valChkStr(dojo.byId('frmSearchCriteria:scProtocolHidden').value);
	...
	if(scProtocolHidden!="")
		if (scText.length > 0)
			scText += " AND " + "(" + scProtocolHidden + ")";
		else
			scText = "(" + scProtocolHidden + ")";
    if (scText.length > 0) 
...
<% // EsriItaly: Button for multilanguage search %>
<h:panelGrid columns="2">
	<h:outputLabel for="scMulti" value="#{gptMsg['catalog.search.search.Multilingua']}"/>
	<h:selectBooleanCheckbox id="scMulti"/> 
</h:panelGrid>
<% // EsriItaly End %>
...
<h:panelGrid columns="4">
  ...
  <h:inputText id="scText"
               ...
               maxlength="4000" styleClass="searchBox form-control" />
  <h:commandButton id="btnDoSearch" rendered="true" styleClass="btn btn-default form-control"
...
<h:panelGrid columns="1" id="scPnlSpatial">
  ...
  <h:panelGrid id="pnlMap">
    <h:panelGroup id="mapToolbar" styleClass="mapToolbar" style="display:none">
      ...
      <h:selectOneMenu id="scSel" styleClass="form-control"
      ...
      <h:inputText id="mapInput-locate" styleClass="locatorInput form-control"
...
<h:panelGroup id="_pngCtypeLocal">
  ...
  <h:panelGrid id="scPnlContent">
    <h:selectOneMenu id="scSelContent" styleClass="form-control"
...
<%//Added By Esri Italy:find ContentType by protocol value%>
<h:panelGroup id="_pngProtocol">
  <h:outputText escape="false" value="<h3>"/>
  <h:outputText id="scLblProtocol" value="#{gptMsg['catalog.search.filterContentTypes.title']}" />
  <h:outputText escape="false" value="</h3>"/>
  <h:panelGrid id="scPnlProtocol">
    <h:selectOneMenu id="scSelProtocol" styleClass="form-control"
                     value="#{SearchFilterProtocol.protocol}"
                     onchange="javascript:updateHiddenValue(this, 'frmSearchCriteria:scProtocolHidden')"
                     >
      <f:selectItem itemValue=""
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.default']}" />
      <f:selectItem itemValue="contentType:(*WMS* *WFS* *WCS* *KML* *MAP* *REST*) OR contentType:(*wms* *wfs* *wcs* *kml* *map* *rest*) OR resource.url:(*WMS* *WFS* *WCS* *KML* *KMZ* *REST*) OR resource.url:(*wms* *wfs* *wcs* *kml* *map* *rest*) OR ((*:* NOT protocol:(* TO *)) AND contentType:liveData)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.liveData']}" />
      <f:selectItem itemValue="contentType:(*download*) OR ((*:* NOT protocol:(* TO *)) AND contentType:downloadableData)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.downloadableData']}" />
      <f:selectItem itemValue="contentType:(*information*) OR ((*:* NOT protocol:(* TO *)) AND contentType:geographicService )"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.offlineData']}" />
      <f:selectItem itemValue="contentType:(*:MAPIMG*) OR ((*:* NOT protocol:(* TO *)) AND contentType:staticMapImage )"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.staticMapImage']}" />
      <f:selectItem itemValue="contentType:(*DOC*)  OR ((*:* NOT protocol:(* TO *)) AND (contentType:document OR contentType:*DOC*) ))"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.document']}" />
      <f:selectItem itemValue="contentType:(*WEBAPP*) OR ((*:* NOT protocol:(* TO *)) AND contentType:application)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.application']}" />
      <f:selectItem itemValue="contentType:(*TASK*)  OR ((*:* NOT protocol:(* TO *)) AND contentType:geographicService)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.geographicService']}" />
      <f:selectItem itemValue="contentType:clearinghouse"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.clearinghouse']}" />
      <f:selectItem itemValue="contentType:mapFiles"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.mapFiles']}" />
      <f:selectItem itemValue="contentType:geographicActivities"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.geographicActivities']}" />
      <f:selectItem itemValue="contentType:(*STATICIMG*)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.staticImage']}" />
      <f:selectItem itemValue="contentType:(*AUDIO*)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.audio']}" />
      <f:selectItem itemValue="contentType:(*VIDEO*)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.video']}" />
      <f:selectItem itemValue="contentType:(*METEO*)"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.meteo']}" />
      <f:selectItem itemValue="contentType:unknown"
                    itemLabel="#{gptMsg['catalog.search.filterContentTypes.unknown']}" />
    </h:selectOneMenu>
  </h:panelGrid>
</h:panelGroup>
...
<h:panelGroup id="_pngCtypeRemote">
  ...
  <h:panelGrid id="scPnlContentR">
    <h:selectOneMenu id="scSelContentR" styleClass="form-control"
...
<h:panelGroup id="_pngModDateSection">
  ...
  <h:panelGrid columns="3" id="scPnlDate">
    ...
    <h:inputText id="scDateFrom" maxlength="10" styleClass="form-control"
    ...
    <h:inputText id="scDateTo" maxlength="10" styleClass="form-control"
                 onchange="javascript:updateHiddenValue(this)"
...
<h:panelGroup id="_pngSortSection">
  ...
  <h:panelGrid id="scPnlSort">
    <h:selectOneMenu id="scSelSort" styleClass="form-control"
...
<%/*******************Modified by Esri Italy************************************
#{SearchController.searchCriteria.searchFilterContentTypes.selectedContentType}*/ %>
...
<%//Added By Esri Italy%>
<h:inputHidden id="scProtocolHidden" value="#{SearchFilterProtocol.protocol}"/>
