...
<h1 class="title_custom"><%=com.esri.gpt.framework.jsf.PageContext.extractMessageBroker(
                                                                          ).retrieveMessage("catalog.publication.uploadMetadata.caption")%></h1>
...
<h:panelGrid columns="1" summary="#{gptMsg['catalog.general.designOnly']}"
  ...
  <%//Added by ESRI Italy
	  //<h:commandButton id="recognize"
      //value="#{gptMsg['catalog.publication.uploadMetadata.button.recognize']}"
      //action="#{ValidateMetadataController.getNavigationOutcome}"
     // actionListener="#{UploadMetadataController.processAction}">
      //<f:attribute name="command" value="recognize" />
   //</h:commandButton>%>
  </h:panelGroup>
...
<%/*ESRI 14/03/2017 sostituizione dettata dalle impostazioni utente*/%>
   <h:panelGroup>
    <h:selectBooleanCheckbox value="#{UploadMetadataController.replaceIfExist}" id="resplaceIfExist"/>
    <h:outputLabel for="resplaceIfExist" value="#{gptMsg['catalog.publication.uploadMetadata.replaceIfExist']}"/>
  </h:panelGroup>
