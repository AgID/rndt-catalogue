...
public class proxy : IHttpHandler {
    ...
    private static string PROXY_REFERER = "./sproxy/proxy.ashx";
	...
	private string proxyAddress = "";//"http://192.168.189.1:808";
    private string[] byPassProxyList ;//{ "win-vjqvssu8pr5", "vm10030-enelags", "localhost" };
    ...
    public void ProcessRequest(HttpContext context) {
        ...
        try {
			...
			proxyAddress = ProxyConfig.GetCustomProxyUrl();
			byPassProxyList = ProxyConfig.GetCustomProxyByPassArray();
        ...
        if (!passThrough && !String.IsNullOrEmpty(serverUrl.Domain))
        {
			log(TraceLevel.Info, "Dominio Presente o Vuoto!! ");
            if (serverUrl.Username.Equals(""))
            {
				
                log(TraceLevel.Info, "Create DEFAULT credential ");//mp
                credentials = System.Net.CredentialCache.DefaultNetworkCredentials;		
				
            }
            else
            {
                log(TraceLevel.Info, "Create credential for " + serverUrl.Domain + "\\" + serverUrl.Username);//mp
                credentials = new System.Net.NetworkCredential(serverUrl.Username, serverUrl.Password, serverUrl.Domain);
            }
        }
        else
        {
			log(TraceLevel.Info, "Dominio non impostato!! ");
            ...
            {
				log(TraceLevel.Info, "passThrough && hasClientToken OK");
				...
				log(TraceLevel.Info, "Token = " + token);
            }
...
*/
    ...
    private System.Net.WebResponse doHTTPRequest(string uri, byte[] bytes, string method, string referer, string contentType, System.Net.NetworkCredential credentials = null)
    {
		...
		log(TraceLevel.Info, "proxyAddress: " + proxyAddress);
		...
		if(proxyAddress != null){
			...
			if(byPassProxyList != null && byPassProxyList.Length > 0){ 
				myProxy.BypassList = byPassProxyList;
			}
		...
		log(TraceLevel.Info, "credentials: " + credentials);
    ...
    private string getNewTokenIfCredentialsAreSpecified(ServerUrl su, string reqUrl) {
		...
		log(TraceLevel.Info, "getNewTokenIfCredentialsAreSpecified (su,reqUrl):" + "(" + su + "," + reqUrl + ")");
...
public class ProxyConfig
{
	...
	//CustomProxy byPassProxyList   
    public static String[] GetCustomProxyByPassArray()
    {
		//log(TraceLevel.Info, "-- GetAllowedReferersArray --");//mp
        if (byPassProxyList == null || byPassProxyList.ToString().Trim().Length==0)
            return null;

        return byPassProxyList.Split(',');
    }
    public static String GetCustomProxyUrl()
    {
		if (customProxyUrl == null || useCustomProxy == false ||  customProxyUrl.ToString().Trim().Length==0)
            return null;
	...
	static String byPassProxyList;
	static String customProxyUrl;
	...
	//customProxy ByPassList
    [XmlAttribute("customProxyByPassList")]
    public string ByPassProxyList
    {
        get { return byPassProxyList; }
        set
        {
            byPassProxyList = Regex.Replace(value, @"\s", "");
        }
    }	
    //customProxy Url
    [XmlAttribute("customProxyUrl")]
    public String CustomProxyUrl
    {
        get { return customProxyUrl; }
        set
        { customProxyUrl = value; }
    }
	
    [XmlAttribute("useCustomProxy")]
